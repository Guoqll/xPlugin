...
abstract class BaseAnAction(actionText: String) : AnAction(actionText) {
    ...
    private fun initBaseInfo(selectGroup: VirtualFile?, className: String) {
        filePath = selectGroup!!.path + "/${className.toLowerCase()}"
        packageName = filePath.substring(filePath.indexOf("java") + 5, filePath.length).replace("/", ".")
        mainPath = filePath.substring(0, filePath.indexOf("java"))
    }
