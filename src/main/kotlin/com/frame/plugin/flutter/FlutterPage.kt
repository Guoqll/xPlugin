package com.frame.plugin.flutter
...
class FlutterPage : AnAction("FlutterPage") {
    ...
    private fun createMvpBase(className2: String) {
        ...
        val filePath = selectGroup!!.path + "/${className.toLowerCase()}"
        val packageName = filePath.substring(filePath.indexOf("lib") + 4, filePath.length).replace("/", ".")
        val mainPath = filePath.substring(0, filePath.indexOf("lib"))
...
//        val classPageName = fileName.replace("(.)([A-Z])".toRegex(), "$1_$2").lowercase(Locale.getDefault())
        PluginUtil.writetoFile(page, className, "${fileName}.dart")

        //======FlutterPageState
        var viewmodel = PluginUtil.readFile("BaseFlutterPageViewModel.txt")!!
        viewmodel = viewmodel.replace("&Home&", "$className2")

        PluginUtil.writetoFile(viewmodel, className, "${output2}.dart")
